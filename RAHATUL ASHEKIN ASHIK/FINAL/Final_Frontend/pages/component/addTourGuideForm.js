...
const AddTourGuideForm = () => {
    ...
    const handleChange = (e) => {
        setTourGuideData({ ...tourGuideData, [e.target.name]: e.target.value });
    };

    const handleSubmit = async (e) => {
        e.preventDefault();

        tourGuideData.contact = parseInt(tourGuideData.contact);
        tourGuideData.age = parseInt(tourGuideData.age);
        console.log(tourGuideData);

        try {
            const response = await axios.post(process.env.NEXT_PUBLIC_API_BASE_URL + '/register/tourguide', tourGuideData, {
                headers: {
            ...
            console.error('Error Tour Guide Registration:', error);
            alert("Tour Guide Registration Failed!");
        }
    };
    ...
    return (
        <>
            ...
            <div class="flex flex-wrap justify-center z-1 relative">
                <div>
                    <h3 class="text-center mb-4 text-3xl font-bold text-black"> Tour Guide Information </h3>
