...
export class TourGuideService{
   ...
   async regTourGuide(tourguideRegInfo : TourGuideRegDTO): Promise<TourGuideEntity> {
    ...
    async addTraveller (travellerRegInfo:TravellerRegDTO, tourguidename:string) : Promise<TravellerEntity> {
            ...
            travellerRegInfo.tourgudieID = tourguide.id;
    
            return this.travelerRepo.save(travellerRegInfo);
        }


    async getTourGuideinfoByname(tourguideUsername:string)
    {
        console.log(tourguideUsername);
        const tourgudie = await this.tourguideRepo.findOneBy({name:tourguideUsername});
    ...
    async receivePaymentByname(paymentreceiveInfo:PaymentReceiveDTO): Promise<PaymentEntity> {
        // const data = await this.paymentRepo.findOneBy({name:tourgudiename});
        // paymentreceiveInfo.tourguideID= data.id;
    
      return this.paymentRepo.save(paymentreceiveInfo);

    }

    async getallpayment(viewallpaymenInfo: PaymentReceiveDTO): Promise<PaymentEntity[]>{
        return this.paymentRepo.find();
    }

    async createtourpack(createtourpackInfo:CreateTourPackDTO, tourguidename:string): Promise<createTourPackEntity> {
       const tourgudie = await this.tourpackRepo.findOneBy({name:tourguidename});
       createtourpackInfo.tourguideID=tourgudie.id;
        return this.tourpackRepo.save(createtourpackInfo);
    }

    async getbookhotelandvehicle(hotalandvechicleInfo:HotelandVehicleDTO): Promise<createhotelandvehivleEntity> {
    ...
    async sendmailtotraveller(messageInfo: TourgudieMessageDTO, tourguidename: string) {
        ...
        await this.mailerService.sendMail(
            {
                to:"hasanmollah151@gmail.com",
                subject:"Tourgudie:"+ tourguideName+":"+messageInfo.subject,
